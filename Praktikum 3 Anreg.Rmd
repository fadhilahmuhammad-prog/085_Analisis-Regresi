...
```{r}
library(readxl)
dt <- read.csv2("C:\\Users\\ASUS\\OneDrive\\문서\\DAFFA\\IPB\\STK\\ANREG\\DATA_RUMAH_JABODETABEK_BERSIH.csv")
dt
```

```{r}
y <- as.numeric(dt$"price_in_rp")
x <- dt$"bedrooms"
n <- length(dt)
n
```
...
$$E[\hat{Y}] = \hat{Y} = 50.3533 + 2.87498 X$$
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [48.14893 ,  52.55772] dan untuk parameter $β_1$ adalah [2.661897 ,  3.088056]
...
```{r}
# Cek data training (yang dipakai membuat model)
class(x) 
# Kemungkinan output: "character" atau "factor"

# Cek data baru (yang menyebabkan error)
class(y)     
# Kemungkinan output: "numeric" atau "integer"
```
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 79.10309. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 75.74925 hingga 82.45694.